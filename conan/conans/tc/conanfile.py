...
class OptHelper:
	...
	def get_onoff(s, opt):
		return 'ON' if s.is_on(opt) else 'OFF'

	def is_on(s, opt):
		if isinstance(opt, PackageOptionValue):
			return opt in [True, 'ON', 1]

		return opt._value in ['ON', True, 1]

	def is_off(s, opt):
		if isinstance(opt, PackageOptionValue):
			return opt in [False]

		return opt._value in ['OFF', False, 0]
...
class SourceHelper:
	...
	def do_source(self):
		...
		for file in glob(f'*-{self.version}'):
