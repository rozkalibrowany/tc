...
#include <tc/server/asio/AsioService.h>
...
int main(int argc, char** argv)
{
	...
	tc::LogI log(logger);
	LG_NFO(log.logger(), "TCP server port");
	...
	while (true) {
		...
		LG_NFO(log.logger(), "sessionsSize: {} payloadPackets size {}", server->sessionsSize(), (int) server->payloadPackets().size());
	}
